...
RC Table::make_record(int value_num, const Value *values, Record &record)
{
  ...
  for (int i = 0; i < value_num; i++) {
    ...
    if (field->type() != value.attr_type() && (!field->nullable() && value.attr_type()==NULLS)) {
  ...
  for (int i = 0; i < value_num; i++) {
    ...
    LOG_WARN("%s",value.get_string().c_str());
...
RC Table::create_index(Trx *trx, const FieldMeta *field_meta, const char *index_name,bool unique)
{
  ...
  if (rc != RC::SUCCESS) {
    ...
    index->close();
    persistHandler.remove_file(index_file.c_str());
    return rc;
  }
